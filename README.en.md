...
[Українська](README.md) | **English** | [C# binary module](AOT/README.md)
...
| `-Language en|uk` | Select installer and TerminalAI UI language. |
...
Without a prompt, `ai` shows its quick-reference card. It also recognizes management subcommands such as `help`, `status`, `models`, `model <name>`, `lang <en|uk>`, `font`, `fonts`, `alias`, and `config`.
...
| `/lang <en|uk>` | Change session language; `permanent` persists it. |
...
Configuration is stored in `~/.terminal-ai/config.json`, or under the directory specified by `TERMINAL_AI_CONFIG_DIR`. `TERMINAL_AI_LANG` takes precedence for interface language.
...
| `Language` | `en` | UI and response language (`en` or `uk`). |
...
TerminalAI.psm1
  -> TerminalAiConfig.ps1       configuration, language, fonts
...
> [!CAUTION]
> The current P2 installer fixtures use temporary profile/module/fragment paths, but their early installer calls do not set `TERMINAL_AI_CONFIG_DIR`; they can update the real `~/.terminal-ai/config.json`. Back up that file or set the environment variable before running those suites.
